...
export default function AchievementsPage() {
  ...
  const { data: all } = useListAchievements();
  const { data: earned } = useGetPlayerAchievements(playerId!, {
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground font-bold">
              {earned?.length ?? 0} / {all?.length ?? 0} unlocked
            </p>
          </div>
        </div>
        {/* Progress bar */}
        <div className="h-3 bg-muted rounded-full overflow-hidden">
          <motion.div
            ...
            animate={{
              width: `${
                all?.length
                  ? ((earned?.length ?? 0) / all.length) * 100
                  : 0
              }%`,
            }}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {a.rewardMoney && a.rewardMoney > 0 ? (
                  <span className="text-xs font-bold bg-accent/20 text-accent px-2 py-0.5 rounded-full mt-1 inline-block">
                    +${a.rewardMoney}
                  </span>
                ) : null}
              </div>
              {isEarned && (
                <span className="text-xs font-black text-accent shrink-0">Unlocked</span>
              )}
            </motion.div>
          );
        })}
