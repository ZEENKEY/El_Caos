      ...
      { id: missionId, data: { playerId } },
      {
        onSuccess: (res) => {
          ...
          toast({
            ...
            description: `Ganaste $${res.rewardEarned ?? 0} pesos.`,
          });
        },
        onError: () => {
          toast({
    ...
    <div className="p-4 max-w-2xl mx-auto space-y-6">
      ...
      {!missions || missions.length === 0 ? (
        <div className="text-center py-16 space-y-3">
          ...
          <p className="text-xl font-black text-muted-foreground">No missions yet</p>
        </div>
      ) : (
        <div className="space-y-4">
          {active.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-base font-black uppercase text-muted-foreground tracking-wider">Active</h2>
              {active.map((m, i) => (
                <motion.div
                  key={m.id}
                      ...
                      <div className="flex gap-2 flex-wrap">
                        <span
                          className={`text-xs font-bold px-2 py-1 rounded-full ${
                            typeColors[m.type ?? ""] ?? "bg-muted text-muted-foreground"
                          }`}
                        >
                          {m.type}
                        </span>
          ...
          {completed.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-base font-black uppercase text-muted-foreground tracking-wider">Completed</h2>
              {completed.map((m) => (
                <div
                  key={m.id}
                  className="bg-card rounded-2xl p-4 border border-border shadow-sm opacity-50 flex items-center gap-3"
                >
