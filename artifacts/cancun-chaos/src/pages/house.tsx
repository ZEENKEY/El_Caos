...
import { ArrowLeft, Home, Hammer, Star, Zap } from "lucide-react";
...
export default function HousePage() {
  ...
  const { data: house, refetch } = useGetHouse(playerId!, {
  ...
  const handleUpgrade = () => {
    ...
    if ((player?.money ?? 0) < nextCost) {
      toast({
        variant: "destructive",
        title: "Sin dinero",
        description: `Necesitas $${nextCost} pesos para mejorar.`,
      });
      return;
    }
    updateHouse.mutate(
      { playerId: playerId!, data: { style: nextStyle } },
      {
        ...
        onError: () => {
          toast({
            variant: "destructive",
            title: "Error",
            description: "No se pudo mejorar la casa.",
          });
        },
      }
    );
    ...
    <div className="p-4 max-w-2xl mx-auto space-y-6">
      ...
      {!house ? (
        <div className="text-center py-16 space-y-3">
          ...
          <p className="font-black text-muted-foreground">Loading house...</p>
        </div>
      ) : (
        <>
          <motion.div
            ...
            className="bg-card rounded-3xl p-8 border border-border shadow-xl text-center space-y-4"
          >
            <motion.div
              animate={{ y: [0, -8, 0] }}
              ...
              {STYLES.map((_, i) => (
                <div
                  ...
                  className={`h-2 rounded-full transition-colors ${
                    i <= level ? "bg-primary flex-[2]" : "bg-muted flex-1"
                  }`}
                />
              ))}
            </div>
            <p className="text-muted-foreground font-bold">
              Level {level + 1} / {STYLES.length}
            </p>
            ...
            <div className="grid grid-cols-3 gap-3 text-center text-sm">
              ...
              <div className="bg-muted rounded-xl p-3">
                <p className="font-black text-lg">
                  <Star size={16} className="inline text-accent" /> {house.visitCount}
                </p>
                <p className="text-muted-foreground font-bold">Visits</p>
              </div>
            </div>
          </motion.div>
          ...
          {isMaxLevel ? (
            <div className="bg-accent/10 border border-accent rounded-2xl p-6 text-center">
              <p className="text-2xl font-black text-accent">MAX LEVEL</p>
              <p className="text-muted-foreground font-bold mt-1">
                Tu casa es una leyenda del barrio.
              </p>
            </div>
          ) : (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-black text-lg">
                    Upgrade to {STYLE_NAMES[level + 1]}
                  </h3>
                  <p className="text-muted-foreground font-bold text-sm">
                    Cost: ${nextCost} pesos — You have: ${player?.money ?? 0}
                  </p>
                </div>
              </div>
              <Button
                ...
                disabled={updateHouse.isPending || (player?.money ?? 0) < nextCost}
                className="w-full font-black text-lg py-6 rounded-2xl shadow"
              >
                <Hammer className="mr-2" size={20} />
                {updateHouse.isPending ? "Upgrading..." : `Upgrade — $${nextCost}`}
              </Button>
            </div>
          )}
        </>
