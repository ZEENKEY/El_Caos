...
export default function MinigamePage() {
  ...
  const getAccuracy = (pos: number): { label: string; score: number } => {
    const center = Math.abs(pos - 0.5);
    if (center <= (zones.perfect ?? 0.1) / 2)
      return { label: "PERFECTO", score: 100 };
    if (center <= (zones.good ?? 0.25) / 2)
      return { label: "BIEN", score: 70 };
    if (center <= (zones.ok ?? 0.4) / 2)
      return { label: "OK", score: 40 };
    return { label: "FAIL", score: 10 };
  };
  ...
  const handleClick = () => {
    ...
    const { label, score } = getAccuracy(current);

    setLastAccuracy(label);
    setLastScore(score);
    setPhase("result");
    ...
    playMinigame.mutate(
      {
        id: minigame.id,
        data: { playerId, score, accuracy: label.toLowerCase() },
      },
      {
        onSuccess: (res) => {
  ...
  return (
    <div className="p-4 max-w-lg mx-auto space-y-6 select-none">
      ...
      <div className="bg-card rounded-3xl p-8 border border-border shadow-lg text-center space-y-4">
        ...
        <h1 className="text-3xl font-black uppercase text-primary tracking-tight">
          {minigame.name}
        </h1>
        ...
        <div className="flex justify-center gap-4 text-sm font-bold">
          <span className="bg-accent/20 text-accent px-3 py-1 rounded-full">
            Reward: ${minigame.baseReward}
          </span>
          <span className="bg-primary/20 text-primary px-3 py-1 rounded-full">
            Difficulty: {"★".repeat(minigame.difficulty ?? 1)}
          </span>
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        {phase === "intro" && (
          ...
          >
            <p className="text-lg font-bold text-muted-foreground">
              Stop the bar as close to the center as possible.
              <br />
              The more centered, the bigger the reward.
            </p>
            <Button
              size="lg"
              onClick={() => setPhase("playing")}
              className="text-xl font-black px-12 py-8 rounded-2xl shadow-lg"
            >
              <Gamepad2 className="mr-3" size={28} /> START
            </Button>
          </motion.div>
        )}
        ...
        {phase === "playing" && (
          ...
          >
            ...
            >
              {/* Perfect zone */}
              <div
                className="absolute top-0 bottom-0 bg-accent/40 rounded"
                style={{
                  left: `${(0.5 - (zones.perfect ?? 0.1) / 2) * 100}%`,
                  width: `${(zones.perfect ?? 0.1) * 100}%`,
                }}
              />
              {/* Good zone */}
              <div
                ...
                style={{
                  left: `${(0.5 - (zones.good ?? 0.25) / 2) * 100}%`,
                  width: `${(zones.good ?? 0.25) * 100}%`,
                }}
              />
              {/* Moving bar */}
              <motion.div
                className="absolute top-1 bottom-1 w-4 bg-primary rounded-xl shadow-lg"
                style={{ left: `calc(${barPos * 100}% - 8px)` }}
                transition={{ type: "linear" }}
              />
              {/* Center line */}
              <div className="absolute left-1/2 top-0 bottom-0 w-0.5 bg-foreground/20" />
            </div>
            <p className="text-center text-2xl font-black text-primary animate-pulse">
              TAP TO STOP!
            </p>
            <div className="flex justify-center gap-4 text-xs font-bold text-muted-foreground">
              <span className="flex items-center gap-1">
                <span className="inline-block w-3 h-3 rounded bg-accent/40" /> Perfect
              </span>
              <span className="flex items-center gap-1">
                <span className="inline-block w-3 h-3 rounded bg-secondary/20" /> Good
              </span>
            </div>
          </motion.div>
        )}
        ...
        {phase === "result" && (
          ...
          >
            <div
              className={`text-6xl font-black ${accuracyColor[lastAccuracy ?? ""] ?? ""}`}
            >
              {lastAccuracy}
            </div>
            ...
            <div className="flex gap-3 justify-center">
              ...
              <Button onClick={() => setLocation("/game")} className="font-black">
                Back to Map
              </Button>
            </div>
          </motion.div>
