    ...
    makeDecision.mutate(
      { playerId, data: { decisionId, choice } },
      {
        onSuccess: (res) => {
          refetchDecisions();
          toast({
            title: "Decision tomada!",
            description: res.consequence ?? "Consecuencias inminentes.",
          });
        },
        onError: () => {
          toast({
            variant: "destructive",
            title: "Error",
            description: "No se pudo procesar la decision.",
          });
        },
      }
    );
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-secondary text-secondary-foreground rounded-3xl p-6 shadow-lg flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="opacity-80 font-bold">Gossip & decisiones</p>
          </div>
        </div>
        <Button
          variant="outline"
      ...
      <div className="flex gap-2">
        <Button
          variant={activeTab === "gossip" ? "default" : "outline"}
        ...
        <Button
          ...
          className="font-black flex-1 relative"
        >
          ...
          >
            {!gossip || gossip.length === 0 ? (
              <div className="text-center py-12 space-y-3">
                ...
                <p className="text-xl font-black text-muted-foreground">Sin chismes</p>
                <p className="text-muted-foreground text-sm">
                  Genera el primero — el barrio esta aburrido.
                </p>
              </div>
            ) : (
              gossip.map((g, i) => (
          ...
          >
            {pendingDecisions.length === 0 ? (
              <div className="text-center py-12 space-y-3">
                ...
                <p className="text-muted-foreground text-sm">
                  Explora el mapa para que pasen cosas.
                </p>
              </div>
            ) : (
              pendingDecisions.map((d, i) => (
                <motion.div
                  ...
                  className="bg-card rounded-2xl p-5 border border-border shadow space-y-4"
                >
                  <h3 className="font-black text-lg">{d.title}</h3>
                  {d.description && (
                    <p className="text-muted-foreground text-sm font-medium">{d.description}</p>
                  )}
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      onClick={() => handleDecision(d.id, "A")}
                      disabled={makeDecision.isPending}
                      className="bg-primary/10 hover:bg-primary/20 border-2 border-primary/30 text-primary rounded-xl p-4 text-left transition-colors"
                    >
                    ...
                    <button
                      ...
                      disabled={makeDecision.isPending}
                      className="bg-secondary/10 hover:bg-secondary/20 border-2 border-secondary/30 text-secondary rounded-xl p-4 text-left transition-colors"
                    >
